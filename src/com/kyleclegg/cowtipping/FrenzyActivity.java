...
public class FrenzyActivity extends Activity implements OnClickListener {
	...
	TextView tvTimer;
	ImageView cow1, cow2, cow3, cow4, cow5, cow6, cow7, cow8, cow9, cow10,
			cow11, cow12, cow13, cow14;
	ImageView cows[];
	...
	/**
	 * Random number generating, which will be used to set the number of clicks
	 * it takes to tip the cow, ranging from 1 to 5
	 */
	...
	/**
	 * Random number generating, which tells us which cow noise to play Just so
	 * it doesn't get boring to hear the same moo over and over :)
	 * 
	 ...
	 * possible on computer). Did some research and found support that switch
	 * statements with a small range like this are more time-efficient
	 * 
	 * Follow-up - After adding the switch it appears the number of audio
	 * warnings (write blocked for xx seconds)
	 */
	...
	private void initializeCows() {
		...
		/**
		 * Create and manually add the ImageViews to an array to be more easily
		 * accessible
		 */
		...
		/**
		 * Create the Map that will be used to track the number of clicks per
		 * cow the map will store an ImageView as the key and an int (num of
		 * clicks) as the value
		 */
		 ...
		 * using the method omNumberofClicks() Decided to store in an array this
		...
		for (int i = 0; i < cowclicks.length; i++) {
			...
			// Set number of clicks to 2 for frenzy mode
			cowclicks[i] = 1;

		}

		/**
		 * Lastly, restore any changed images or animations Note - this will
		 * only apply to re-populated cows
		 */
	...
	public void onClick(View v) {
		 ...
		 * tasks based on the cow clicked and the number of times it has been
		 * clicked
		 * 
		 * Each switch case has 5 parts 1 - Increase the value by one to track
		 * the number of clicks 2 - If the number of clicks = random number - 1
		 * then change image 2 - If the number of clicks = random number
		 * then...3,4,5 3 - Make a mooing noise 4 - Animate the tipping of the
		 * cow 5 - Set Visibility to 0 6 - Increase the count 7 - Populate new
		 * cows
		 * 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/**
		 * Begin background farm and game noises The farm noises were found on a
		 * free mp3 sites and are public domains Mission Impossible - Ragtime
		 * was also found on a free mp3 site
		 */
		...
		mRotateAnimation90 = AnimationUtils.loadAnimation(this,
				R.anim.rotate_90);
		mRotateAnimation180 = AnimationUtils.loadAnimation(this,
				R.anim.rotate_180);
		mTranslateAnimationRight = AnimationUtils.loadAnimation(this,
				R.anim.translate_right);
		mTranslateAnimationLeft = AnimationUtils.loadAnimation(this,
				R.anim.translate_left);
		mFadeAwayAnimation = AnimationUtils.loadAnimation(this,
				R.anim.fade_away);
				...
				if (done) {
					...
					rotate.setAnimationListener(new AnimationListener() {
						...
						public void onAnimationEnd(Animation animation) {
							finish();
							Intent intent = new Intent(FrenzyActivity.this,
		 ...
		 * In initializeCows we: 1 - Create ImageViews and assign them to our
		 * images 2 - Add to an array 3 - Create a Map to track number of clicks
		 * 4 - Set a listener for all s 5 - Get a random number of clicks for
		 * each
		 */
		 ...
		 * First, even though we've created all the cows we'll start by showing
		 * none Then, we'll populate the cows randomly using the populateMany()
		 * call
		 */
	...
	public void onPause() {
		...
		try {
			...
			Log.d("ONPAUSE",
					"Could not pause media (probably because it is no longer playing)");
		}
	...
	public void onResume() {
		...
		/**
		 * Create timer to countdown activity, decided that it should go in
		 * onResume
		 */
		...
		if (this.timerTimeLeft > 0) {
			...
			final Toast hint = Toast.makeText(this,
					"HINT: Tap a cow repeatedly until it moos!",
					Toast.LENGTH_SHORT);
	...
	private void toastScores() {

		/**
		 * First, toast the score!
		 */

		if (count == 0) {
			Toast cowsTipped = Toast.makeText(this, "Done! " + count
					+ " cows tipped!", Toast.LENGTH_SHORT);
			cowsTipped.show();
			// extra toast for tipping zero
			Toast zeroTipped = Toast.makeText(this,
					"Yikes! Better luck next time!", Toast.LENGTH_SHORT);
			zeroTipped.show();
		} else if (count < 2) {
			Toast cowsTipped = Toast.makeText(this, "Done! " + count
					+ " cow tipped!", Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast oneTipped = Toast.makeText(this, "You can do better!",
					Toast.LENGTH_SHORT);
			oneTipped.show();
		} else if (count < 4) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast twoTipped = Toast.makeText(this,
					"Try again. You can tip more than that!",
					Toast.LENGTH_SHORT);
			twoTipped.show();
		} else if (count < 6) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanFiveTipped = Toast.makeText(this,
					"Not bad, now let's see some real tippin'!",
					Toast.LENGTH_SHORT);
			...
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanEightTipped = Toast.makeText(this,
					"Pretty good, for an amateur!", Toast.LENGTH_SHORT);
			lessThanEightTipped.show();
		} else if (count < 10) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTenTipped = Toast.makeText(this,
					"Keep up the good work!", Toast.LENGTH_SHORT);
			lessThanTenTipped.show();
		} else if (count < 12) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTwelveTipped = Toast.makeText(this,
					"You are the bomb!", Toast.LENGTH_SHORT);
			lessThanTwelveTipped.show();
		} else if (count < 14) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanFourteenTipped = Toast.makeText(this,
					"Call the sheriff we have a cow tipper on the loose!",
					Toast.LENGTH_SHORT);
			...
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanSixteenTipped = Toast.makeText(this,
					"Very impressive!", Toast.LENGTH_SHORT);
			lessThanSixteenTipped.show();
		} else if (count < 20) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTwentyTipped = Toast.makeText(this,
					"Holy Shnikeys! You're awesome!", Toast.LENGTH_SHORT);
			lessThanTwentyTipped.show();
		} else if (count < 24) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTwentyFourTipped = Toast.makeText(this,
					"All hail the Cow Tipper!", Toast.LENGTH_SHORT);
			...
			cowsTipped.show();
			Toast lessThanTwentyFourTipped = Toast.makeText(this,
					"Wow! You're hotter than socks on a rooster!", Toast.LENGTH_SHORT);
			lessThanTwentyFourTipped.show();
		} else if (count < 40) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTwentyFourTipped = Toast.makeText(this,
					"Out of this world!", Toast.LENGTH_SHORT);
			lessThanTwentyFourTipped.show();
		} else if (count < 50) {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast lessThanTwentyFourTipped = Toast.makeText(this,
					"You are THE master cow tipper!", Toast.LENGTH_SHORT);
			lessThanTwentyFourTipped.show();
		}
		else {
			Toast cowsTipped = Toast.makeText(this, count + " cows tipped!",
					Toast.LENGTH_SHORT);
			cowsTipped.show();
			Toast aLotTipped = Toast.makeText(this, "Unbelieveable! You Rock!",
					Toast.LENGTH_SHORT);
			aLotTipped.show();
		}

		/**
		 * Save top 5 high scores as preferences. Here, check to see if this is
		 * the highest score, if yes congratulate and update! If top 5, save
		 */

		if (count > highScore1) {
			Toast highScore = Toast.makeText(this, "New High Score!",
					Toast.LENGTH_SHORT);
			highScore.show();
	...
	/**
	 * Populate many will be called in onCreate and will populate the field with
	 * many cows ready to be tipped It's okay to just set visibility here
	 */
	...
	/**
	 * PopulateSome will be called when there are less that 3 cows left on the
	 * field Must create new cows, not just set to visible
	 */
